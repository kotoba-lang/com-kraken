(ns kraken.main
  "Kotodama WASM entrypoint for the Kraken clean-room actor (L4) — Clojure port.
  ...
  facts (`kraken.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kraken")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "kraken_acc"
    :fields [:holderId :currency :balance :status]      :required [:currency :balance]
    :coerce {:balance :float}             :refs {}}
   {:entity "Card"        :plural "cards"        :id-prefix "kraken_car"
    :fields [:accountId :last4 :network :state]         :required [:last4 :network]
    :coerce {}                            :refs {:accountId "Account"}}
   {:entity "Transaction" :plural "transactions" :id-prefix "kraken_tra"
    :fields [:accountId :amount :currency :type :status] :required [:amount :currency]
    :coerce {:amount :float}              :refs {:accountId "Account"}}
   {:entity "Order"       :plural "orders"       :id-prefix "kraken_ord"
    :fields [:symbol :side :quantity :price :status]    :required [:symbol :side]
    :coerce {:quantity :float :price :float} :refs {}}
   {:entity "Wallet"      :plural "wallets"      :id-prefix "kraken_wal"
    :fields [:address :chain :balance]                  :required [:address :chain]
    :coerce {:balance :float}             :refs {}}
   {:entity "Ledger"      :plural "ledgers"      :id-prefix "kraken_led"
    :fields [:accountId :debit :credit :memo]           :required [:debit :credit]
    :coerce {:debit :float :credit :float} :refs {:accountId "Account"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kraken.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kraken-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kraken-compat/ready)
